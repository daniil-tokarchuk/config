...
alias cdm="cd ${PROJECTS}/dom-monorepo"
...
alias csp="ssh root@65.108.196.190"
alias csd="ssh root@65.109.146.239"

alias cor="codex -C /Users/smuzi/Projects/dom-monorepo mcp login"
alias cora="cor atlassian"
alias corn="cor notion"

alias uds="${PROJECTS}/dom-monorepo/.agent/skills/update-submodules/update.sh"
alias udb="${PROJECTS}/dom-monorepo/.agent/skills/build-all/build.sh"

gdom() {
  if [[ $# -eq 0 ]]; then
    echo "Error: No 'git' subcommand provided." >&2
    echo "Usage: gdom <subcommand> [args...]" >&2
    return 1
  fi
  local BASE_DIR="${HOME}/Projects/dom-monorepo"
  for repo in "$BASE_DIR"/dom-*; do
    [[ -d $repo ]] || continue
    local name=$(basename "$repo")
    case "$name" in dom-database|dom-config) continue ;; esac
    (cd "$repo" && echo "- $name" && git "$@")
  done
}
